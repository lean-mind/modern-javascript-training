...
<Notes>
  ...
  - ¿Qué se considera JavaScript moderno? Podemos definir como JavaScript moderno al JavaScript a partir de ES2015, 
    ya que a partir de esta versión el lenguaje dió un salto de calidad.
  - Esto sumado al compromiso por parte de Ecma international y la comunidad de JavaScript de lanzar una nueva versión cada año añadiendo las nuevas funcionalidades.
  - Todo esto junto con Node.js conllevó a una explosión en cuanto al uso del lenguaje y herramientas del ecosistema.
  - Todo esto permite un desarrollo en JavaScript más rápido y seguro que antes.
...
<Notes>
  ...
  - Antes de 2015 JavaScript como lenguaje no tenía manera de dividir el código en ficheros o módulos e ir importándolos entre ellos para reusar código y no tener un fichero kilométrico. Existían librerías como RequireJS que permitían, pero no era ningún estándar.
  - Con 2015 llegaron los conocidos como ES Modules para dar una forma estandarizada en el lenguaje para importar y exportar código entre ficheros JavaScript.
...
<Notes>
  ...
  - Vamos a hablar muy brevemente de la asincronía.

</Notes>

---

## Asincronía - Antes de las promesas
...
<Notes>
  ...
  - La gestión de asincronía antes de 2015 se hacía por callbacks, lo cual hacía todo muy difícil de debuggear y leer, ya que se originaban los famosos callback hells.
...
<Notes>
  ...
  - Toda función con la keyword async devuelve una promesa. Dicho de otra manera, toda función asíncrona devuelve una promesa.
...
<Notes>
  ...
  - El await siempre debe ser usado dentro de una función asíncrona (En Node.js llegará en breve la opción de poder usarlo a nivel global).
...
<Notes>
  ...
  - Devuelve el primer elemento que resuelva satisfactoriamente la condición.
...
```javascript title="Object methods (entries)"
  const invoice = {
    id: 'SUP0001',
    total: 254.13,
    foo() {
  ...
  Object.entries(invoice) // [['id', 'SUP0001'], ['total', 254.13], ['foo', ƒ()]]

  const list = ['Bat Man', 'Superman', 'Green Lintern', 'Wonder Woman']
  Object.entries(list) // [[0, 'Bat Man'], [1, 'Superman'], [2, 'Green Lintern'], [3, 'Wonder Woman']]
```
...
<Notes>
  ...
  - Devuelve una matriz de pares clave valor de un objeto
  - Si no se utiliza con un objeto, realiza una coerción (ej, listas)
...
```javascript title="Object methods (values)"
  const invoice = {
    id: 'SUP0001',
    total: 254.13,
    foo() {
  ...
  Object.values(invoice) // ['SUP0001', 254.13, ƒ()]
  Object.keys(invoice) // ['id', 'total', 'foo']
```
...
<Notes>
  ...
  - Devuelve una lista cuyos elementos son los valores de las propiedades
  - Object.keys() ya existía antes de 2015, pero es complementario
...
```javascript title="Destructuring (spread operator)"
  const money = { amount: 1_000_000_000_000, currency: '¥' }
  const wallet = { owner: '欽明天皇', ...money }

  wallet // { owner: '欽明天皇', amount: 1_000_000_000_000, currency: '¥' }
```
...
<Notes>
  ...
  - 欽明天皇 - Kinmei Tennoo (Emperador Kinmei)
...
```javascript title="Destructuring (rest parameter)"
  const wallet = { owner: '欽明天皇', amount: 1_000_000_000_000, currency: '¥' }
  const { owner, ...money } = wallet

  owner // '欽明天皇'
  money // { amount: 1_000_000_000_000, currency: '¥' }
```
...
```javascript title="Classes"
  ...
  const pidgeon = new Bird('Pidgeot')
...
```javascript title="Enhanced object literals"
  const currency = 'EUR'
  const type = 'withdraw'
...
```javascript title="Optional chaining"
  const anselmo = {
    name: 'Anselmo'
  }
  const antonio = {
  ...
  console.log(anselmo?.cat?.name)
  console.log(antonio?.cat?.name)
